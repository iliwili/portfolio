...
public class GetMe : IQuery<AuthUserDto>
{
}

public class GetMeHandler(DatabaseContext databaseContext, ILogger<GetMeHandler> logger, ICurrentUser currentUser) : IQueryHandler<GetMe, AuthUserDto>
{
    public async ValueTask<AuthUserDto> Handle(GetMe query, CancellationToken cancellationToken)
    {
        try
        {
